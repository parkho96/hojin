...
try{document.title = $('#menuPath').val().replace(/>/gi,'-') + '<spring:message code="wzwg.cmm.word.regist" />';}catch(e){console.log(e.message);}
	...
	$(document).ready(function(){
		...
		$('#regist_btn').click(function(){
			...
			if(!confirm('<spring:message code="wzwg.cmm.cmmMsg.CMG007"><spring:argument><spring:message code="wzwg.cmm.word.stre" text="save" /></spring:argument></spring:message>')){
				...
				if('<c:out value="${editorEstbsSe}"/>' == 'S'){
			        ...
			        var nttCn = frm.nttCn.value ;
			        nttCn = nttCn.replace(/<(\/?)p>/gi,"");
			        nttCn = nttCn.replace(/(<br>)|(<br \/>)/gi,"");
			        nttCn = nttCn.replace(/\s/gi,"");
			        nttCn = nttCn.replace(/&nbsp;/gi,"");


					if(nttCn == '' || nttCn.length == 0){
				...
				$.ajax({
			        type : 'POST'
					, url : '<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/qna/registNttInfoAjax.do'
					, cache : false
					, async : false
					, processData: false
					, contentType: false
		...
		$('#subospec_add_btn').click(function(){
			...
			frm.action = "<c:out value='${wzwg_contextPath}${prefix}'/>/module/bbs/qna/selectBbsInc.do";
			frm.submit();
		});
		...
		$('#file_upload_btn').click(function(){
			 ...
			 $.ajax({
		        type : 'POST'
				, url : '<c:out value="${wzwg_contextPath}${prefix}"/>/module/upload/file/fileForm.do'
				, dataType : 'html'
				, success : function (data) {
					$("#bbs_layer").show();
			    	$("#bbs_layer").html(data);
	...
	function fnFileUploader(){
		...
		<form:form modelAttribute="paramVO" path="regFrm" id="regFrm" name="regFrm" method="post">
			...
			<div class="register-box">
				<div class="subject">
					<table>
					<caption id="contentsCaption"><spring:message code="wzwg.module.word.postregist" /></caption>
					<colgroup>
						<col width="10%"/>
						<col width="15%"/>
						<col width="13%"/>
					...
					<c:if test="${paramVO.subospecSeq ne null}">
						...
						<tr>
							...
							<td  colspan="4">
								...
								<ul class="setlist">
									<c:if test="${adminAuthAt eq 'Y'}">
										<li><label><form:checkbox path="noticeAt" name="noticeAt" value="Y" dir="required" title="FAQ ${fn:escapeXml(faqRegTit)}" /> <span>FAQ <spring:message code="wzwg.cmm.word.regist" /></span></label></li>
									</c:if>
			...
			<div class="btnbox-c">
				<a href="javascript:void(0);" class="wzbtn btn-del" id="cancle_btn"><spring:message code="wzwg.cmm.word.cancl" /></a>
 				<a href="javascript:void(0);" class="wzbtn btn-basic" id="tmpr_regist_btn" class="two"><spring:message code="wzwg.cmm.word.temsve" text="temporary save" /></a>
