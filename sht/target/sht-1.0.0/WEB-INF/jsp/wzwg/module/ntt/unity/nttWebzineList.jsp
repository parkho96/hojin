...
try{document.title = $('#menuPath').val().replace(/>/gi,'-') + ' <spring:message code="wzwg.cmm.word.list" />';}catch(e){console.log(e.message);}
	...
	window.onkeydown = function() {
		...
		<form:form modelAttribute="paramVO" path="listFrm" id="listFrm" name="listFrm" method="post">
			...
			<c:set var="fileAt" value="${resultVO.atchFilePosblAt}" />
			<c:set var="fileCo" value="${resultVO.atchFilePosblCo}" />
			
			<form:hidden path="atchFilePosblAt" value="${fn:escapeXml(fileAt)}" />
			<form:hidden path="atchFilePosblCo" value="${fn:escapeXml(fileCo)}" />
				...
				<c:if test="${resultVO.listCountAt eq 'Y'}">
					...
					<form:select path="listCount" name="listCount" id="listCount" cssStyle="width:80px;margin-bottom:0px;" title="${fn:escapeXml(listCntSeTit)}">
						<c:forEach items="${countList}" var="countList">						
							<c:set var="count" value="${countList}" />
							<form:option value="${fn:escapeXml(count)}">
						        <c:out value="${fn:escapeXml(count)}"/><spring:message code="wzwg.cmm.word.count03" />
						    </form:option>
						</c:forEach>
					</form:select>
					</div>
							...
							<h3 class="fw600">
								...
								<span>
									<c:if test="${resultList.lv > 1}">
										<c:forEach step="1" begin="1" end="${resultList.lv - 1}">
											<img src="/images/wzwg/module/ntt/icoReply.png" class="mg_r2"/>
										</c:forEach>
									</c:if>
								</span>								
								...
								<c:choose>
									<c:when test="${listDetAuthAt eq 'Y'}">
										<a href="javascript:void(0);" onclick="fnView('<c:out value="${resultList.nttSeq}"/>', '<c:out value="${resultList.ntcrId}"/>', '<c:out value="${resultList.secretAt}"/>', '<c:out value="${resultList.parntsNttSeq}"/>','<c:out value="${resultList.parntsNtcrId}"/>');"><c:out value="${resultList.nttSj}"/></a>
										<c:if test="${resultList.answerCnt > 0}">
											<span class="colorRed">[<c:out value="${resultList.answerCnt}"/>]</span>
										</c:if>											
									</c:when>
									<c:otherwise>
										<a href="javascript:void(0);" onclick="alert('<spring:message code="wzwg.cmm.msg.MSG084" />');"><c:out value="${resultList.nttSj}"/></a>		
										<c:if test="${resultList.answerCnt > 0}">
											<a href="javascript:void(0);" onclick="alert('<spring:message code="wzwg.cmm.msg.MSG084" />');">
												<span class="colorRed">[<c:out value="${resultList.answerCnt}"/>]</span>
											</a>
							...
							<div class="conTop">
								<p class="writer">
									<span>
										...
										<c:if test="${resultList.annymtyAt ne 'Y'}"><c:out value="${resultList.ntcrNm}"/>(<c:out value="${fn:substring(resultList.ntcrId, 0, 2)}"/>****)</c:if>
									</span>
								</p>
								<p class="list_date">
