...
try{document.title = $('#menuPath').val().replace(/>/gi,'-') + '<spring:message code="wzwg.cmm.word.list" />';}catch(e){console.log(e.message);}
	...
	$(document).ready(function(){
		...
		$('#btn_subospecSeq').click(function(){
			...
			$.ajax({
				  type : 'POST'
				, dataType: 'html'
				, url : '<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/qna/selectNttFaqListAjax.do'
				, cache : false
				, async : false
				, data:$("#listFrm").serialize()
				, success:function (data) {
	...
	window.onkeydown = function() {
		...
		<form:form modelAttribute="paramVO" path="listFrm" id="listFrm" name="listFrm" method="post">
			...
			<h5 id="subospecSj_txt" class="hide-txt"><c:out value="${paramVO.subospecSj}"/><c:if test="${empty paramVO.subospecSj}"><spring:message code="wzwg.cmm.word.faq" /></c:if></h5>
			...
			<div class="btnbox-r wd100 mt30">
				...
				<c:if test="${delAuthAt eq 'Y'}">
					<a href="javascript:void(0);" class="wzbtn btn-del fl" id="delete_btn"><spring:message code="wzwg.cmm.word.delete" /></a>
				</c:if>
				...
				<c:if test="${faqRegAuthAt eq 'Y'}">
					<a href="javascript:void(0);" class="wzbtn btn-basic" id="faq_delete_btn"><spring:message code="wzwg.module.word.faqexcl" /></a>
				</c:if>
