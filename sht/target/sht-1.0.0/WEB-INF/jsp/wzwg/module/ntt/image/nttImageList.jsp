...
if($(location).attr('href').indexOf('/cmnt') > -1) {
	...
	window.onkeydown = function() {
		...
		<form:form modelAttribute="paramVO" path="listFrm" id="listFrm" name="listFrm" method="post">
			...
			<form:hidden path="atchImgFilePosblCo" value="${fn:escapeXml(resultVO.atchImgFilePosblCo)}" />
			...
			<c:if test="${!empty resultList}">
				...
				<!-- album -->
				<c:if test="${resultVO.listCountUnit eq 3 }">
				<c:set var="listCss">nth3</c:set>
				</c:if>
				<c:if test="${resultVO.listCountUnit eq 4 }">
				<c:set var="listCss">nth4</c:set>
				</c:if>
				
				<c:if test="${mobileAt eq 'Y'}">
					<ul class="album ${listCss }">
				</c:if>				
		
				<c:forEach var="resultList" items="${resultList}" varStatus="status">
							
				<c:if test="${mobileAt eq 'N'}">
					<c:if test="${status.count % paramVO.listCount == 1}">
						<ul class="album ${fn:escapeXml(listCss)}">
					</c:if>
				</c:if>
						...
						<c:choose>
							...
							<span class="imgBox">
								<c:if test="${paramVO.listScrinCode eq 'I'}">
									<c:if test="${not empty resultList.atchFileId}">
										<img src="<c:url value='/module/upload/file/selectImageView.do'/>?atchFileId=${fn:escapeXml(resultList.atchFileId)}&ran=${fn:escapeXml(ran)}" alt="${fn:escapeXml(resultList.fileDc)}" />
									</c:if>
							...
							<c:choose>
								...
								<c:otherwise>				
									...
									<c:if test="${resultList.answerCnt > 0}">
										<a href="javascript:void(0);"  onclick="alert('<spring:message code="wzwg.cmm.msg.MSG084" />');">
											<span class="reflyTxt"><i class="fa fa-comment-o" aria-hidden="true"></i><span class="blind"><spring:message code="wzwg.module.word.registanswerco" /></span><span class="rfno"><c:out value="${resultList.answerCnt}"/></span></span>
											<!-- <span class="colorRed">[${resultList.answerCnt}]22222222222</span> -->
										</a>
									</c:if>								
								</c:otherwise>
							</c:choose>			
				...
				<c:if test="${mobileAt eq 'N'}">
				<c:if test="${status.count % paramVO.listCount == 0 or status.last}">
					</ul>
				</c:if>
				</c:if>
			...
			<c:if test="${empty resultList}">
			<ul>
				<li><div class="ctr-box pt30 pb50"><spring:message code="wzwg.cmm.msg.MSG085" /></div></li>
			</ul>
			</c:if>
			...
			<div class="ctr-box" id="nttSearch">
				...
				<form:select path="searchCondition" id="searchCondition" title="${searchTit}"> 
				...
				<form:input path="searchKeyword" id="searchKeyword" placeholder="${srchwrd}" cssClass="txt" onkeydown="if(event.keyCode == 13){fnPage(1);}" title="${searchkeyinp}"/>	
