...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$('#btn_subospecSeq').click(function(){
			...
			$.ajax({
				  type : 'POST'
				, dataType: 'html'
				, url : '<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/image/selectNttRecycleListAjax.do'
				, cache : false
				, async : false
		...
		$('#recycle_btn').click(function(){
			if(!confirm('<spring:message code="wzwg.cmm.cmmMsg.CMG007"><spring:argument><spring:message code="wzwg.cmm.word.recovry" /></spring:argument></spring:message>')){
				...
				$.ajax({
  					type:'POST'
					, url:'<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/image/modifyNttRecycleAjax.do'
					, cache : false
					, async : false
					, data:$("#listFrm").serialize()
	...
	function fnDeleteNtt() {
		if(!confirm('<spring:message code="wzwg.cmm.msg.MSG060" />')){
			...
			$.ajax({
					type:'POST'
				, url:'<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/image/deleteNttAjax.do'
				, cache : false
				, async : false
				, data:$("#listFrm").serialize()
				, success:function (result) {
	...
	function fnDeleteNttAll() {
		...
		if(!confirm(msg + '\n' + '<spring:message code="wzwg.cmm.msg.MSG015" />')){
			...
			$.ajax({
					type:'POST'
				, url:'<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/image/deleteNttAllAjax.do'
				, cache : false
				, async : false
				, data:$("#listFrm").serialize()
				, success:function (result) {
	...
	window.onkeydown = function() {
		...
		<form:form modelAttribute="paramVO" path="listFrm" id="listFrm" name="listFrm" method="post">
			...
			<c:set var="temp_cmntUseAt" value="${resultVO.cmntUseAt}" />
			<input type="hidden" name="cmntUseAt" id="cmntUseAt" value="<c:out value='${temp_cmntUseAt}'/>" />
					...
					<div class="alBox">
							...
							<span class="imgBox">
								<c:if test="${paramVO.listScrinCode eq 'I'}">
									<c:set var="temp_nttSj" value="${resultList.nttSj}" />
									<c:set var="temp_atchFileId" value="${resultList.atchFileId}" />
								
									<c:if test="${resultList.atchFileCnt ne '0'}">
										<img src="<c:url value='/module/upload/file/selectImageView.do'/>?atchFileId=<c:out value='${temp_atchFileId}'/>" alt="<c:out value='${temp_nttSj}'/>" />
									</c:if>
									<c:if test="${resultList.atchFileCnt eq '0'}">
										<img src="/images/egovframework/com/noimg_img.gif" alt="<c:out value='${temp_nttSj}'/>" />
									</c:if>
								</c:if>
							</span>
						...
						<p>
							...
							<c:if test="${adminAuthAt eq 'Y'}">
								...
								<input type="checkbox" name="nttChk" id="nttChk" value="<c:out value='${temp_nttSeq}'/>" title="<spring:message code="wzwg.cmm.word.rowchoise" />"/>				
							</c:if>						
