...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$('#recycle_btn').click(function(){
			if(!confirm('<spring:message code="wzwg.cmm.cmmMsg.CMG007"><spring:argument><spring:message code="wzwg.cmm.word.recovry" /></spring:argument></spring:message>')){
				...
				$.ajax({
  					type:'POST'
					, url:'<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/link/modifyNttLinkRecycleAjax.do'
					, cache : false
					, async : false
					...
					, success:function (result) {
						...
						if(value == 'success'){
							alert('<spring:message code="wzwg.cmm.cmmMsg.CMG005"><spring:argument><spring:message code="wzwg.cmm.word.recovry" text="recovery" /></spring:argument></spring:message>');
							fnPage(1);
						}else{
	...
	function fnDeleteNtt() {
		if(!confirm('<spring:message code="wzwg.cmm.msg.MSG060" />')){
			...
			$.ajax({
					type:'POST'
				, url:'<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/link/deleteNttLinkAjax.do'
				, cache : false
				, async : false
				, data:$("#listFrm").serialize()
				, success:function (result) {
	...
	function fnDeleteNttAll() {
		...
		if(!confirm(msg + '\n' + '<spring:message code="wzwg.cmm.msg.MSG015" />')){
			...
			$.ajax({
					type:'POST'
				, url:'<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/link/deleteNttLinkAllAjax.do'
				, cache : false
				, async : false
				, data:$("#listFrm").serialize()
				, success:function (result) {
	...
	function fnPage(pageIndex){
		...
		$.ajax({
			  type : 'POST'
			, dataType: 'html'
			, url : '<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/link/selectNttLinkRecycleListAjax.do'
			, cache : false
			, async : false
			, data:$("#listFrm").serialize()
					...
					<div class="event-txt">
						...
						<c:choose>
							<c:when test="${!empty resultList.nttSj}">
								<c:if test="${fn:length(resultList.nttSj) > 35}">
									<c:set var="nttSj" value="${fn:substring(resultList.nttSj, 0, 35)}..." />
								</c:if>
								<c:if test="${fn:length(resultList.nttSj) < 36}">
						...
						<h3><c:out value="${nttSj}"/></h3>
						<p class="list_date">
							<c:out value="${resultList.frstRegistPnttm}"/>
						</p>
						<div class="comM_txt">
							<c:out value='${resultList.linkDc}' escapeXml="false" />
						</div>
					</div>
