...
if($(location).attr('href').indexOf('/cmnt') > -1) {
	...
	// 상세정보
	function fnView(nttSeq, ntcrId, secretAt, parntsNttSeq, parntsNtcrSeq, ntcrSeq){
				...
				<li style="width:100%;">
						...
						<div class="imgbox">
							<c:choose>
								<c:when test="${listDetAuthAt eq 'Y'}">
									<a href="javascript:void(0);" onclick="fnView('<c:out value="${resultList.nttSeq}"/>', '<c:out value="${resultList.ntcrId}"/>', '<c:out value="${resultList.secretAt}"/>', '<c:out value="${resultList.parntsNttSeq}"/>','<c:out value="${resultList.parntsNtcrSeq}"/>', , '<c:out value="${resultList.ntcrSeq}"/>');">				
								</c:when>
								<c:otherwise>												
					...
					<div class="event-txt">
						...
						<div class="eventPosTit">
							...
							<c:choose>
								<c:when test="${listDetAuthAt eq 'Y'}">
									<a href="javascript:void(0);" onclick="fnView('<c:out value="${resultList.nttSeq}"/>', '<c:out value="${resultList.ntcrId}"/>', '<c:out value="${resultList.secretAt}"/>', '<c:out value="${resultList.parntsNttSeq}"/>','<c:out value="${resultList.parntsNtcrSeq}"/>', '<c:out value="${resultList.ntcrSeq}"/>');"><c:out value="${nttSj}"/></a>								
						...
						<c:if test="${expsrAtRgsdAt eq 'Y'}">
							<p class="postrestInfo">${resultList.frstRegistPnttm}</p>
						</c:if>
					</div>
