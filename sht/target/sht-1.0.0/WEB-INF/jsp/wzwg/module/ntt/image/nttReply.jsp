...
if($(location).attr('href').indexOf('/cmnt') > -1) {
	if(cmntNm && cmntMenuNm) {
		try{document.title = cmntNm+'-'+cmntMenuNm+'-<spring:message code="wzwg.cmm.word.answer03" />';}catch(e){console.log(e.message);}	
	}else {
		try{document.title = '<spring:message code="wzwg.cmm.word.answer03" />';}catch(e){console.log(e.message);}
	}
}else {
	...
	$(document).ready(function(){
		...
		$('#regist_btn').click(function(){
			...
			if(!confirm('<spring:message code="wzwg.cmm.cmmMsg.CMG007"><spring:argument><spring:message code="wzwg.cmm.word.stre" text="save" /></spring:argument></spring:message>')){
				...
				$.ajax({
			        type : 'POST'
					, url : '<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/image/registNttInfoAjax.do'
	...
	function fnTagInput(tagNm){
		...
		if(inputVal.value.indexOf(tagNm) > -1){
			alert('<spring:message code="wzwg.cmm.msg.MSG318" />');
			return;
		}
	...
	function fnFileUploader(){
		...
		<form:form modelAttribute="resultVO" path="replyFrm" id="replyFrm" name="replyFrm" method="post">
			...
			<c:set var="temp_ctgryTit"><spring:message code="wzwg.module.word.ctgryse" /></c:set>
			<c:set var="temp_nttSjTit"><spring:message code="wzwg.module.word.nttsj" /></c:set>
			<c:set var="temp_notiChekTit"><spring:message code="wzwg.module.word.noticececk" /></c:set>
			<c:set var="temp_secretPostTit"><spring:message code="wzwg.module.word.secretpostsceck" /></c:set>
			<c:set var="temp_annymtyPostTit"><spring:message code="wzwg.module.word.annymtypostsceck" /></c:set>
			<c:set var="temp_answerPermTit"><spring:message code="wzwg.module.word.answerpermceck" /></c:set>
			
			<c:set var="ctgryTit"><c:out value="${temp_ctgryTit}" /></c:set>
			<c:set var="nttSjTit"><c:out value="${temp_nttSjTit}" /></c:set>
			<c:set var="notiChekTit"><c:out value="${temp_notiChekTit}" /></c:set>
			<c:set var="secretPostTit"><c:out value="${temp_secretPostTit}" /></c:set>
			<c:set var="annymtyPostTit"><c:out value="${temp_annymtyPostTit}" /></c:set>
			<c:set var="answerPermTit"><c:out value="${temp_answerPermTit}" /></c:set>
