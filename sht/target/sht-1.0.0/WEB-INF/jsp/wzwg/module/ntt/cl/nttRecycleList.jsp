...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$('#recycle_btn').click(function(){
			if(!confirm('<spring:message code="wzwg.cmm.cmmMsg.CMG007"><spring:argument><spring:message code="wzwg.cmm.word.recovry" /></spring:argument></spring:message>')){
				...
				$.ajax({
  					type:'POST'
					, url:'<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/cl/modifyNttRecycleAjax.do'
					, cache : false
					, async : false
	...
	function fnDeleteNtt() {
		if(!confirm('<spring:message code="wzwg.cmm.msg.MSG060" />')){
			...
			$.ajax({
					type:'POST'
				, url:'<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/cl/deleteNttAjax.do'
				, cache : false
				, async : false
				, data:$("#listFrm").serialize()
				, success:function (result) {
	...
	function fnDeleteNttAll() {
		...
		if(!confirm(msg + '\n' + '<spring:message code="wzwg.cmm.msg.MSG015" />')){
			...
			$.ajax({
					type:'POST'
				, url:'<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/cl/deleteNttAllAjax.do'
				, cache : false
				, async : false
				, data:$("#listFrm").serialize()
				, success:function (result) {
	...
	function fnPage(pageIndex){
		...
		$.ajax({
			  type : 'POST'
			, dataType: 'html'
			, url : '<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/cl/selectNttRecycleListAjax.do'
			, cache : false
			, async : false
			, data:$("#listFrm").serialize()
			, success:function (data) {
		...
		<form:form modelAttribute="paramVO" path="listFrm" id="listFrm" name="listFrm" method="post">
			...
			<table class="<c:out value='${tableCss}'/>">
				  ...
				  <thead>
					<tr>
						<c:if test="${adminAuthAt eq 'Y'}">
							<th><input type="checkbox" name="nttAllChk" id="nttAllChk" title="<spring:message code="wzwg.cmm.word.rowallchoise" />"/></th>					
						</c:if>
						<c:if test="${mobileAt eq 'N'}">
							<th><span><spring:message code="wzwg.cmm.word.no" /></span></th>
						</c:if>
						<th><span><spring:message code="wzwg.module.word.clnm" /></span></th>
				  ...
				  <tbody>		  
					...
					<c:if test="${!empty resultList}">
						...
						<tr>
							...
							<c:if test="${adminAuthAt eq 'Y'}">
								<c:set var="nttSeq"><c:out value="${resultList.nttSeq}" /></c:set>
								<td><input type="checkbox" name="nttChk" id="nttChk" value="${fn:escapeXml(nttSeq)}" title="<spring:message code="wzwg.cmm.word.rowchoise" />"/></td>				
							</c:if>						
							...
							<c:set var="clNmNoneTit"><spring:message code="wzwg.module.word.clnmnone" /></c:set>							
							<td class="txt-l">								
								<c:choose>
									...
									<c:otherwise>
										<c:set var="nttSj" value="${clNmNoneTit}" />
									</c:otherwise>
								</c:choose>
					...
					<c:if test="${empty resultList}">
						<tr>
							<c:set var="colCnt"><c:out value="${colCnt}" /></c:set>
			...
			<div class="ctr-box" id="nttSearch">
				...
				<form:select path="searchCondition" id="searchCondition" title="${fn:escapeXml(searchTit)}"> 
					<form:option value="1"><label for="option3"><spring:message code="wzwg.module.word.clnm" />+<spring:message code="wzwg.cmm.word.cn" /></label></form:option>
					<form:option value="2"><label for="option3"><spring:message code="wzwg.module.word.clnm" /></label></form:option>
