...
<script type="text/javascript">
	...
	function fnTabLink(val){
		...
		if(val == 'bassInfo'){
			pageUrl = '<c:out value="${wzwg_contextPath}${prefix}"/>/module/bbs/mvp/selectMvpBbsBassInfoAjax.do';
		}
		
		if(val == 'dataManage'){			
			if("<c:out value='${param.pmode}'/>" == "detail") {
				document.getElementById('mvpnttSeq').value = "<c:out value='${param.mvpnttSeq}'/>";
				pageUrl = '<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/mvp/selectNttMvpDetailAjax.do';
			} else {
				pageUrl = '<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/mvp/selectNttMvpListAjax.do';
			}
		} 
		...
		if(val == 'dataRecycle'){
			pageUrl = '<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/mvp/selectNttMvpRecycleListAjax.do';
		}			
		...
		<c:if test="${mngrAt eq 'Y'}">
	    	...
	    	<select id="bbsSel" class="w20 mg_b30" onchange="fnBbsChage(this.value);">
				<c:forEach var="resultList" items="${bbsList}">
					...
					<c:if test="${fn:indexOf(resultList.pckagePath, 'mvp') > -1}">
