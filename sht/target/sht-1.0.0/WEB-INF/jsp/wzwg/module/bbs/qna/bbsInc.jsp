...
<link type="text/css" href='/css/wzwg/module/ntt/basic/qnaBoard.css' rel="stylesheet">
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		//$('head').find('link[href="/css/wzwg/site/mngr/style.css"]').remove();
	...
	function fnTabLink(val){
		...
		if(val == 'bassInfo'){
			pageUrl = '<c:out value="${wzwg_contextPath}${prefix}"/>/module/bbs/qna/selectQnaBbsBassInfoAjax.do';
		}
		
		if(val == 'dataManage'){
			if("<c:out value='${param.pmode}'/>" == "detail") {
				document.getElementById('nttSeq').value = "<c:out value='${param.nttSeq}'/>";
				pageUrl = '<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/qna/selectNttDetailAjax.do';
			} else {
				pageUrl = '<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/qna/selectNttListAjax.do';
			}			
		}
        ...
        if(val == 'dataRecycle'){
			pageUrl = '<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/qna/selectNttRecycleListAjax.do';
		}	
		...
		<c:if test="${mngrAt eq 'Y'}">
	    	...
	    	<select id="bbsSel" class="w20 mg_b30" onchange="fnBbsChage(this.value);">
				<c:forEach var="resultList" items="${bbsList}">
					...
					<c:if test="${fn:indexOf(resultList.pckagePath, 'qna') > -1}">
