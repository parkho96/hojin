...
<link type="text/css" href='/css/wzwg/module/ntt/basic/faqBoard.css' rel="stylesheet" id="css_link">
...
<script type="text/javascript">
	...
	function fnTabLink(val){
		...
		if(val == 'bassInfo'){
			pageUrl = '<c:out value="${wzwg_contextPath}${prefix}"/>/module/bbs/faq/selectFaqBbsBassInfoAjax.do';
		}
		
		if(val == 'dataManage'){			
			if("<c:out value='${param.pmode}'/>" == "detail") {
				document.getElementById('nttSeq').value = "<c:out value='${param.nttSeq}'/>";
				pageUrl = '<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/faq/selectNttDetailAjax.do';
			} else {
				pageUrl = '<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/faq/selectNttListAjax.do';
			}
		}
		...
		if(val == 'dataRecycle'){
			pageUrl = '<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/faq/selectNttRecycleListAjax.do';
		}		
		...
		<c:if test="${mngrAt eq 'Y'}">
	    	...
	    	<select id="bbsSel" class="w20 mg_b30" onchange="fnBbsChage(this.value);">
				<c:forEach var="resultList" items="${bbsList}">
					...
					<c:if test="${fn:indexOf(resultList.pckagePath, 'faq') > -1}">
