...
<link type="text/css" href='/css/wzwg/module/ntt/basic/linkBoard.css' rel="stylesheet">
...
<script type="text/javascript">
	...
	function fnTabLink(val){
		...
		if(val == 'bassInfo'){
			pageUrl = '<c:out value="${wzwg_contextPath}${prefix}"/>/module/bbs/link/selectLinkBbsBassInfoAjax.do';
		}
		
		if(val == 'dataManage'){			
			if("<c:out value='${param.pmode}'/>" == "detail") {
				document.getElementById('nttSeq').value = "<c:out value='${param.nttSeq}'/>";
				pageUrl = '<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/link/selectNttLinkDetailAjax.do';
			} else {
				pageUrl = '<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/link/selectNttLinkListAjax.do';
			}
		}
		...
		if(val == 'dataRecycle'){
			pageUrl = '<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/link/selectNttLinkRecycleListAjax.do';
		}		
		...
		<c:if test="${mngrAt eq 'Y'}">
	    	...
	    	<select id="bbsSel" class="w20 mg_b30" onchange="fnBbsChage(this.value);">
				<c:forEach var="resultList" items="${bbsList}">
					...
					<c:if test="${fn:indexOf(resultList.pckagePath, 'link') > -1}">
