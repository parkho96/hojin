...
<%@ include file="/WEB-INF/jsp/wzwg/cmm/include/incTaglibs.jsp" %>
	...
	<link type="text/css" href='/css/wzwg/module/ntt/basic/simpBoard.css' rel="stylesheet" id="css_link">
...
<script type="text/javascript">
	...
	function fnTabLink(val){
		...
		if(val == 'bassInfo'){
			pageUrl = '<c:out value="${wzwg_contextPath}${prefix}"/>/module/bbs/simp/selectSimpBbsBassInfoAjax.do';
		}
		
		if(val == 'dataManage'){
			pageUrl = '<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/simp/selectNttFormAjax.do';
		}
        ...
        if(val == 'dataRecycle'){
			pageUrl = '<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/simp/selectNttRecycleListAjax.do';
		}	        
		...
		<c:if test="${mngrAt eq 'Y'}">
	    	...
	    	<select id="bbsSel" class="w20 mg_b30" onchange="fnBbsChage(this.value);">
				<c:forEach var="resultList" items="${bbsList}">
					...
					<c:if test="${fn:indexOf(resultList.pckagePath, 'simp') > -1}">
