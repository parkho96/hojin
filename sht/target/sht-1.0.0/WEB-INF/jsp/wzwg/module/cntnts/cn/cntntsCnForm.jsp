			...
			<div class="tableWrap row mg_t20">
				<table class="basic">
					...
					<tbody>
						...
						<tr>
							<th><spring:message code="wzwg.module.word.applctemplate" /></th>
							<td><div id="tmplatDiv"></div></td>
						</tr>
						<tr>
							<th><spring:message code="wzwg.cmm.word.cn" /></th>
							<td>
								<textarea name="cntntsCn" id="cntntsCn" rows="60" class="w90" dir="required" title="<spring:message code="wzwg.cmm.word.cn" />" style="width:100%;">
									<c:if test="${empty paramVO.cntntsCnSeq}"><c:out value="${tmplatVO.cntntsCn }"/></c:if>
									<c:if test="${not empty paramVO.cntntsCnSeq}"><c:out value="${resultVO.cntntsCn }"/></c:if>
								</textarea>
