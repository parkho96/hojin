...
<script>
    $(function () {
    	var labels = [
    		 <c:forEach items="${statList}" var="resultList" varStatus="status">'<c:out value="${resultList.cmntNm}" />' <c:if test="${!status.last}">,</c:if></c:forEach>
			];
		var data = {
				  labels: labels,
				  datasets: [
		...
		var config = {
				  ...
				  options: {
				    ...
				    plugins: {
				      ...
				      title: {
				        display: true,
				        text:  '<spring:message code="wzwg.site.stat.msg.MSG004" />(<spring:message code="wzwg.site.stat.msg.MSG005" />)'
				      }
				    }
				  },
				};
