...
// var nameCheck = /^[_.A-Za-z0-9+]*$/;
 ...
 function fnImgUpload(){
	 ...
	 $('#mngrImgForm').ajaxForm({
		    type:'POST'
		    , url:'<c:out value="${wzwg_contextPath}" />/sysMngr/screen/registScreenTempltResorceFileAjax.do'
		    , beforeSend: function() {
		        //status.empty();
		        var percentVal = '0%';
		    ...
		    , success: function(data) {
		        ...
		        if(data.head.result == 'success'){
			        ...
			        for (var i = 0 ; i < fileList.length; i++){
		            	...
		            	html += '		<button type="button" class="btn-a" onclick="fnImgPrevewPop(\'tmp_img_' + filename + '\')">' + wz_msg('wzwg.cmm.word.preview') + '</button>          ';
		            	html += '		<button type="button" class="btn-b" onclick="fnImgDelete(\'img_item_' + filename + '\' , \'' + item.fileName + '\')">' + wz_msg('wzwg.cmm.word.delete01') + '</button>                                                        ';
		        		html += '	</div>                                                                                                                                                    ';
		        		html += '</li>                                                                                                                                                        ';
			        }
			        //var imgItem = $(html);
	...
	<input type="hidden" name="imgSrc"/>
				...
				<div class="pop-conts">
					 ...
					 <div class="pop-tab fileMngr">
				            <input id="tab_css" type="radio" name="add_layer_tab" checked="checked" />
				            <input id="tab_img" type="radio" name="add_layer_tab" />
				            <!-- <input id="tab_backup" type="radio" name="add_layer_tab" style="display: none;"/> -->
				            <section class="buttons">
				                <label for="tab_css">CSS</label>
				            ...
				            <div class="tab_item layout_select01">
				            	<ul class="mngr_cssList" style="margin-bottom: 15px;">
				            		<c:forEach items="${cssList }" var="list">
				            		<li style="width: 100%; display: block;">
				            			<div class="fileName" style="float: left; width: 70%; line-height: 34px;">
				            			...
				            			<div class="fileControll" style="float: left; width: 29%;">
					            			<button type="button" class="wzbtn btn-basic" onclick="fnLoadTempltCss('/<c:out value="${list.path }${list.fileName }" />', '<c:out value="${list.fileName }" />')"><spring:message code="wzwg.cmm.word.toedit" /></button>
				            			</div>
				            		</li>
				            		</c:forEach>
				            	</ul>
				            	
				            	<div id="cssEditView" style="top: 0; left: 0; width: 100%; background: #fff;">
				            		<form id="mngrCssForm">
				            			<input type="hidden" id="templateSeq" name="templateSeq" value="<c:out value="${paramVO.templateSeq }" />">
					            		<div style="text-align: right;">
					            			<span style="float:left;"><spring:message code="wzwg.sysMngr.word.selectFile" /> : <input type="text" id="cssFileName" name="fileName" style="width: 300px; height: 30px; padding-left: 5px;" readonly="readonly"></span>
					            			<button type="button" class="wzbtn btn-basic" onclick="fnCssSave()"><spring:message code="wzwg.cmm.word.tostre" /></button><button type="button" class="wzbtn btn-basic" onclick="fnCssEditViewToggle()"><spring:message code="wzwg.sysMngr.word.zoomInZoomOut" /></button>
					            		</div>
					            		<div>
					            			<textarea id="cssEdit" name="cssCn" style="width: 100%; height: 300px; top: 0; left: 0; font-size: 13px; line-height: 17px;"></textarea>
					            		</div>
				            		</form>
				            	</div>
				           ...
				           <%--  <div class="tab_item layout_select01">
				            	<form id="mngrBackupForm">
					            	...
					            	<div style="text-align: right;">
					            		<button id="btn_tmplt_backup" type="button" class="btn-a" onclick="fnTempltBackup()">템플릿 백업하기</button>
				            	...
				            	<ul class="mngr_zipList">
				            		...
				            		<li style="width: 100%; display: block;" id="zip_item_${c.count}">
				            			...
				            			<div class="fileControll" style="float: left; width: 29%;">
					            			<a class="btn-a" href="/${list.path }${list.fileName }" target="_blank">다운로드</a>
					            			<button type="button" class="btn-b" onclick="fnZipDelete('zip_item_${c.count}', '${list.fileName }')"><spring:message code="wzwg.cmm.word.delete01" /></button>
				            			</div>
				            		</li>
				            		</c:forEach>
				            	</ul>
