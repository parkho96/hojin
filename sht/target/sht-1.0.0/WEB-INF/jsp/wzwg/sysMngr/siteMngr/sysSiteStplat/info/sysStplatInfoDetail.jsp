...
function fnList() {
    ...
    frm.target = "_self";
    frm.action="<c:out value="${wzwg_contextPath}${prefix}" />/siteMngr/sysSiteStplat/info/selectSysStplatInfoList.do";
    frm.submit();
}
...
function fnModify() {
	...
	$.ajax({
		type:'POST'
		, url:'<c:out value="${wzwg_contextPath}${prefix}" />/siteMngr/sysSiteStplat/info/modifySysStplatInfoAjax.do'
    ...
    $.ajax({
      ...
      , dataType: 'html'
                ...
                <form:form modelAttribute="resultVO" id="regForm" name="regForm" method="post">
					...
					<form:hidden path="siteSeq" value="${fn:escapeXml(resultVO.siteSeq)}" />
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td>
	                                ...
	                                <form:select path="stplatTyCode" name="stplatTyCode" dir="required" title="${fn:escapeXml(msg_title_txt01)}" onchange="fnInit(this.value);">
                            ...
                            <tr>
                            	...
                            	<td> 
                            		<ul class="wzForm wd100">
                            			...
                            			<li><input type="radio" id="essntlN" name="essntlAt" dir="required" value="N" <c:if test="${resultVO.essntlAt eq 'N' }">checked="checked"</c:if>/><label for="essntlN"><spring:message code="wzwg.cmm.word.choise"/></label></li>
                            		</ul>
				...
				<!-- 상세약관 start -->
                <h3 class="wzAdmSTit wd100 fl"><spring:message code="wzwg.sysMngr.word.detail02StplatInfo" /></h3>
                    
                <form:form modelAttribute="resultVO" id="frmInfoSimp" name="frmInfoSimp" method="post">
                <form:hidden path="stplatSeq" name="stplatSeq" value="${fn:escapeXml(resultVO.stplatSeq)}" />
                <form:hidden path="siteSeq" name="siteSeq" value="${fn:escapeXml(resultVO.siteSeq)}" />
                <div id="divInfoSimp">
                ...
                <h3 class="wzAdmSTit wd100 fl"><spring:message code="wzwg.sysMngr.word.detail02StplatHist" /></h3>
