...
<input type="hidden"  name="pageIndex" id="pageIndex" value="0" />
			...
			<div class="main-menu-bar">
				...
				<c:when test="${!empty templtList }">
					<c:forEach items="${templtList}" var="resultList" varStatus="status">
								<li>
									<div>
										<a href="javascript:;" onclick="fn_detail('<c:out value="${resultList.templateSeq}" />')"><img src="/<c:out value="${resultList.templateStreCours }" />screenshot/<c:out value="${resultList.thumbUrl}" />" width="100%"/></a>
									</div>
									<table>
										<colgroup>
											<col width="50%">
										...
										<tr>
											<th colspan="2"><a href="javascript:;" class="wzbtn btn-basic" onclick="fn_detail('<c:out value="${resultList.templateSeq}" />')">상세보기</a></th> 
										</tr>
									</table>
								</li>
								</c:forEach>
	...
	<div class="ctr-box tem-list01">
		<h4><span>03</span><spring:message code="wzwg.sysMngr.word.tmplatAdd" /></h4>
