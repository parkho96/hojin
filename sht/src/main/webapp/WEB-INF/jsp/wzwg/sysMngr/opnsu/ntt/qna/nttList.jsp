...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$('#regist_form_btn').click(function(){
			...
			$.ajax({
				  type : 'POST'
				, dataType: 'html'
				, url : '<c:out value="${wzwg_contextPath}${prefix}" />/opnsu/ntt/qna/registNttFormAjax.do'
				, cache : false
				, async : false
		...
		$('#list_top_regist_btn').click(function(){
			...
			$.ajax({
				type:'POST'    
				, url:'<c:out value="${wzwg_contextPath}${prefix}" />/opnsu/ntt/qna/modifyNttNoticeAjax.do'
				, dataType: 'xml'
				, data:$("#listFrm").serialize()
		...
		$('#subospec_modify_btn').click(function(){
			...
			$.ajax({
		      ...
		      , success:function (data) {
		    	  //$("#bbs_layer").show();
		    	  //$("#bbs_layer").html(data);
		    	  var title = '<spring:message code="wzwg.cmm.word.ctgry02" /> <spring:message code="wzwg.cmm.word.updt" />';
	...
	// 상세정보
	function fnView(nttSeq, ntcrId, secretAt,ntcrSeq){
	...
	function fnSelectSubospec(subospecSeq){
		...
		<form:form modelAttribute="paramVO" path="listFrm" id="listFrm" name="listFrm" method="post">
			...
			<table class="basic-table mngrSuprtTBL suprtQnaBoard">
				  <colgroup>
				  ...
				  <tbody>
					...
					<c:if test="${!empty noticeList}">
						...
						<tr>
							...
							<td><img class="icoCenter" src="/images/wzwg/module/ntt/faq.png" alt="FAQ" /></td>
							
							<c:if test="${adminAuthAt eq 'Y'}">
								<td>
									<a href="<c:out value="http://${noticeList.siteUrl}${port}/" />" target="_blank">
										<c:out value="${noticeList.siteFullNm}"/>
									</a>
								</td>
							</c:if>	
							...
							<td class="txt-l">
								...
								<c:if test="${!empty noticeList.subospecSj}">
									<span class="colorRed">[ <c:out value="${noticeList.subospecSj}" /> ]</span>
								</c:if>					
								...
								<c:choose>
									<c:when test="${noticeDetAuthAt eq 'Y'}">
										...
										<c:if test="${noticeList.answerCnt > 0}">
											<a href="javascript:void(0);" onclick="fnAnswerPop('<c:out value="${noticeList.nttSeq}" />', '<c:out value="${noticeList.ntcrId}" />', '<c:out value="${noticeList.secretAt}" />', '<c:out value="${noticeList.ntcrSeq}" />')">
								...
								<c:if test="${noticeList.nttReplyCnt > 0}">
									<span class="rflyfinished"><spring:message code="wzwg.sysMngr.word.answr01Compt" /></span>
								</c:if>	
							</td>
					...
					<c:if test="${!empty resultList}">
						...
						<tr>
							...
							<c:if test="${adminAuthAt eq 'Y'}">
								<td class="smalltd">
									<a href="<c:out value="http://${resultList.siteUrl}${port}/" />" target="_blank" title="클릭 시 해당 사이트로 이동(새창열림)">
										<c:out value="${resultList.siteFullNm}"/>
									</a>
								</td>
							</c:if>							
							...
							<td class="txt-l">								
								...
								<c:if test="${!empty resultList.subospecSj}">
									[ <c:out value="${resultList.subospecSj}" /> ]
								</c:if>
								...
								<c:choose>
									<c:when test="${listDetAuthAt eq 'Y'}">
										...
										<c:if test="${resultList.answerCnt > 0}">
											<a href="javascript:void(0);" onclick="fnAnswerPop('<c:out value="${resultList.nttSeq}" />', '<c:out value="${resultList.ntcrId}" />', '<c:out value="${resultList.secretAt}" />', '<c:out value="${resultList.ntcrSeq}" />')">
								...
								<c:if test="${resultList.nttReplyCnt > 0}">
									<span class="colorRed"><spring:message code="wzwg.cmm.word.answer01" /> <c:out value="${resultList.nttReplyCnt}" /></span>
								</c:if>
							...
							<td class="smalltd"><c:out value="${resultList.ntcrNm}" /></td>
			...
			<div class="rt-box">
				...
				<c:set var="listTopRegAuthAt" value="" />					
				...
				<c:if test="${listTopRegAuthAt eq 'Y'}">
					<a href="javascript:void(0);" id="list_top_regist_btn" class="wzbtn btn-black"><spring:message code="wzwg.sysMngr.word.faqRegist" /></a>
				</c:if>
