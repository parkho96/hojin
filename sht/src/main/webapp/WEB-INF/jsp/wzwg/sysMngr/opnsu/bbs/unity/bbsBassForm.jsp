...
<script type="text/javascript">
    ...
    function fnRegist(callGubun) {
        ...
        var ajaxUrl = '<c:out value="${wzwg_contextPath}${prefix}" />/opnsu/bbs/unity';
    ...
    function fnReset_btn() {
        ...
        if(!confirm('<spring:message code="wzwg.cmm.msg.MSG305" />')){
            ...
            $.ajax({
                type : 'POST'
                , url : '<c:out value="${wzwg_contextPath}${prefix}" />/opnsu/bbs/unity/selectUnityBbsBassInfoAjax.do'
                , dataType : 'html'
                , data : $("#regForm").serialize()
                , success : function (data) {
        ...
        <form:form modelAttribute="resultVO" path="regForm" id="regForm" name="regForm" method="post">
            ...
            <div class="wzAdmMainSrchbox txt-c mb20">
            	 <input type="text" id="addSubospecSj" name="addSubospecSj" class="w50 ml10" />
	             <a href="javascript:void(0);" id="subospec_add_btn" class="wzbtn btn-black"><spring:message code="wzwg.sysMngr.word.ctgry02Add" /></a>
            </div>
