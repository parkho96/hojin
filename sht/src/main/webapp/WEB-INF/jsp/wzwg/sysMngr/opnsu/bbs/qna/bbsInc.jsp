...
<script type="text/javascript">
	...
	function fnTabLink(val){
		...
		if(val == 'bassInfo'){
			pageUrl = '<c:out value="${wzwg_contextPath}${prefix}" />/opnsu/bbs/qna/selectQnaBbsBassInfoAjax.do';
		}
		
		if(val == 'dataManage'){
			pageUrl = '<c:out value="${wzwg_contextPath}${prefix}" />/opnsu/ntt/qna/selectNttListAjax.do';		
		}
		...
		if(document.getElementById('pmode').value == "detail") {
			...
			pageUrl = '<c:out value="${wzwg_contextPath}${prefix}" />/opnsu/ntt/qna/selectNttDetailAjax.do';
			val = 'dataManage';
		}	
