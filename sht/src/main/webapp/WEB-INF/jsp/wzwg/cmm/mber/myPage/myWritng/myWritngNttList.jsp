...
<script type="text/javascript">
	...
	function fnSearch(callId){
		...
		$.ajax({
			  type : 'POST'
			, dataType: 'html'
			, url : '<c:out value="${wzwg_contextPath}" />/cmm/mber/myPage/selectMyWritngNttListAjax.do'
			, cache : false
			, async : false
			, data:$("#listFrm").serialize()
	...
	<form:form modelAttribute="paramVO" path="listFrm" id="listFrm" name="listFrm" method="post" onsubmit="return false;">
		...
		<caption><spring:message code="wzwg.cmm.word.nttlist" /></caption>
		<colgroup>
			<col width="5%"/>
			<col width="5%">
			<col width="20%">
		...
		<tbody>
			<c:if test="${!empty resultList}">
				...
				<tr>
					...
					<td><c:out value="${resultList.bbsNm}" /></td>
					<td class="txt-l">
						<c:choose>
			...
			<c:if test="${empty resultList}">
				<tr>
					<td colspan="6"><spring:message code="wzwg.cmm.msg.MSG085" /></td>
				</tr>
			</c:if>	
		</tbody>
		</table>
		...
		<c:set var="nttSearchTit"><spring:message code="wzwg.cmm.word.searchse" /></c:set>
		<c:set var="searchkeyinp"><spring:message code="wzwg.cmm.word.searchkeywordinput" /></c:set>
		<div class="ctr-box" id="nttSearch">
			<form:select path="searchCondition" id="searchCondition" title="${fn:escapeXml(nttSearchTit)}">  
