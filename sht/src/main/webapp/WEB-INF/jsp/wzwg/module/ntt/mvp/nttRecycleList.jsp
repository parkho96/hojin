...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$('#subospecSeq').change(function(){
			...
			$.ajax({
				  type : 'POST'
				, dataType: 'html'
				, url : '<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/mvp/selectNttMvpRecycleListAjax.do'
				, cache : false
				, async : false
		...
		$('#recycle_btn').click(function(){
			if(!confirm('<spring:message code="wzwg.cmm.cmmMsg.CMG007"><spring:argument><spring:message code="wzwg.cmm.word.recovry" /></spring:argument></spring:message>')){
				...
				$.ajax({
  					type:'POST'
					, url:'<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/mvp/modifyNttMvpRecycleAjax.do'
					, cache : false
					, async : false
					, data:$("#listFrm").serialize()
	...
	function fnDeleteNtt() {
		if(!confirm('<spring:message code="wzwg.cmm.msg.MSG060" />')){
			...
			$.ajax({
					type:'POST'
				, url:'<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/mvp/deleteNttMvpAjax.do'
				, cache : false
				, async : false
				, data:$("#listFrm").serialize()
				, success:function (result) {
	...
	function fnDeleteNttAll() {
		...
		if(!confirm(msg + '\n' + '<spring:message code="wzwg.cmm.msg.MSG015" />')){
			...
			$.ajax({
					type:'POST'
				, url:'<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/mvp/deleteNttMvpAllAjax.do'
				, cache : false
				, async : false
				, data:$("#listFrm").serialize()
				, success:function (result) {
					...
					<li style="width:100%;">
						...
						<div class="alBox">
								...
								<!-- 
								 ...
								 -->
								<span class="imgBox">
									<img src='http://img.youtube.com/vi/<c:out value="${resultList.mvpKey}"/>/mqdefault.jpg' alt="<c:out value='${resultList.nttSj}'/>" />
								</span>								
							...
							<p>
								...
								<c:if test="${resultList.lv > 1}">
									...
									<img src="/images/wzwg/module/ntt/icoReply.png" style="margin-left:<c:out value='${pd_reply_class}'/>px;" alt="<spring:message code="wzwg.cmm.word.answer03" />" />
								</c:if>
