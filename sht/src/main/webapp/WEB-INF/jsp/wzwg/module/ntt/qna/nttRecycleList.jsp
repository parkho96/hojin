...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$('#recycle_btn').click(function(){
			if(!confirm('<spring:message code="wzwg.cmm.cmmMsg.CMG007"><spring:argument><spring:message code="wzwg.cmm.word.recovry" /></spring:argument></spring:message>')){
				...
				$.ajax({
					type:'POST'
					, url:'<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/qna/modifyNttRecycleAjax.do'
					, cache : false
					, async : false
	...
	function fnDeleteNtt() {
		if(!confirm('<spring:message code="wzwg.cmm.msg.MSG060" />')){
			...
			$.ajax({
					type:'POST'
				, url:'<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/qna/deleteNttAjax.do'
				, cache : false
				, async : false
				, data:$("#listFrm").serialize()
				, success:function (result) {
	...
	function fnDeleteNttAll() {
		...
		if(!confirm(msg + '\n' + '<spring:message code="wzwg.cmm.msg.MSG015" />')){
			...
			$.ajax({
					type:'POST'
				, url:'<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/qna/deleteNttAllAjax.do'
				, cache : false
				, async : false
				, data:$("#listFrm").serialize()
				, success:function (result) {
	...
	function fnPage(pageIndex){
		...
		$.ajax({
			  type : 'POST'
			, dataType: 'html'
			, url : '<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/qna/selectNttRecycleListAjax.do'
			, cache : false
			, async : false
			, data:$("#listFrm").serialize()
			, success:function (data) {
	...
	window.onkeydown = function() {
		...
		<form:form modelAttribute="paramVO" path="listFrm" id="listFrm" name="listFrm" method="post">
				...
				<tbody>
					...
					<c:if test="${empty resultList}">
						<tr>
							...
							<td colspan="<c:out value='${temp_colCnt}'/>"><spring:message code="wzwg.cmm.msg.MSG057" /></td>
						</tr>
					</c:if>						
			...
			<div class="ctr-box" id="nttSearch">
				...
				<input type="button" value="<spring:message code='wzwg.cmm.word.search01' />" class="wzbtn-table btn-srch" onclick="fnPage(1);" title="<spring:message code="wzwg.module.word.searchbutton" />">			 
			</div>					
