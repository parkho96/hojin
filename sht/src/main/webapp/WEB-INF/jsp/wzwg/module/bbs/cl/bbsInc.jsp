...
<%@ include file="/WEB-INF/jsp/wzwg/cmm/include/incTaglibs.jsp" %>
	...
	<link type="text/css" href='/css/wzwg/module/ntt/basic/clBoard.css' rel="stylesheet" id="css_link">
	
	<c:if test="${!empty moduleBbsCssVO}">
		<c:set var="cssFileNm"><c:out value='${moduleBbsCssVO.cssFileNm}'/></c:set>
		<c:set var="cssPath"><c:out value='${moduleBbsCssVO.cssPath}'/></c:set>
...
<script type="text/javascript">
	...
	function fnTabLink(val){
		...
		if(val == 'bassInfo'){
			pageUrl = '<c:out value="${wzwg_contextPath}${prefix}"/>/module/bbs/cl/selectClBbsBassInfoAjax.do';
		}
		
		if(val == 'dataManage'){			
			if("<c:out value='${param.pmode}'/>" == "detail") {
				document.getElementById('nttSeq').value = "<c:out value='${param.nttSeq}'/>";
				pageUrl = '<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/cl/selectNttDetailAjax.do';
			} else {
				pageUrl = '<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/cl/selectNttListAjax.do';
			}
		}
		...
		if(val == 'dataRecycle'){
			pageUrl = '<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/cl/selectNttRecycleListAjax.do';
		}		
		...
		<c:if test="${mngrAt eq 'Y'}">
	    	...
	    	<select id="bbsSel" class="w20 mg_b30" onchange="fnBbsChage(this.value);">
				<c:forEach var="resultList" items="${bbsList}">
					...
					<c:if test="${fn:indexOf(resultList.pckagePath, 'cl') > -1}">
		    ...
		    <div class="step wztab underLine theme-blue adminIcon">
		    	<ul class="tapMenu wztab-list"> 
					...
					<li class="wztab-item"><span class="ico-info"></span><a href="javascript:void(0);" onclick="fnTabLink('dataManage');" 	title="<spring:message code="wzwg.cmm.menu.datamanage" />" 		id="dataManage" 	name="bbsTab"><spring:message code="wzwg.cmm.menu.datamanage" /></a></li>
					</c:if>
					<li class="wztab-item"><span class="ico-data"></span><a href="javascript:void(0);" onclick="fnTabLink('author');" 		title="<spring:message code="wzwg.cmm.menu.author" />" 			id="author" 		name="bbsTab"><spring:message code="wzwg.cmm.menu.author" /></a></li>
					<c:if test="${sessionScope.SYSMNGR_AT ne 'Y'}">
					<li class="wztab-item"><span class="ico-auth"></span><a href="javascript:void(0);" onclick="fnTabLink('charger');" 		title="<spring:message code="wzwg.cmm.menu.charger" />" 		id="charger" 		name="bbsTab"><spring:message code="wzwg.cmm.menu.charger" /></a></li>
