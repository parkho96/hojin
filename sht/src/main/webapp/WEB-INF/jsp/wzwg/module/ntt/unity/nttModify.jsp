	...
	$(document).ready(function(){
		...
		$('#modify_btn').click(function(){
			...
			if(!confirm('<spring:message code="wzwg.cmm.cmmMsg.CMG007"><spring:argument><spring:message code="wzwg.cmm.word.updt" /></spring:argument></spring:message>')){
				...
				$.ajax({
			        type : 'POST'
					, url : '<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/unity/modifyNttInfoAjax.do'
		...
		$('#cancle_btn').click(function(){
			...
			if('<c:out value="${resultVO.listScrinCode}"/>' == 'B'){
				returnUrl = '<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/unity/selectNttListAjax.do';
			}else{
				returnUrl = '<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/unity/selectNttDetailAjax.do';
			}
						...
						<c:if test="${paramVO.atchFilePosblAt eq 'Y'}">
							...
							<c:if test="${fileEstbsSe eq 'B'}">
								...
								<tr>
									...
									<td colspan="4">
										<c:if test="${resultVO.listScrinCode eq 'I'}" >
