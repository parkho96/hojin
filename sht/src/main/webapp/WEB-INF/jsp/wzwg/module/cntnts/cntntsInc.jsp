...
<script type="text/javascript">
	...
	function fnPageReload() {
	    ...
	    frm.action = '<c:out value="${wzwg_contextPath}${prefix}"/>/module/cntnts/selectModuleCntntsInc.do';
	    frm.submit();
	}
	...
	function fnTabChange(paramValue){
		...
		if(paramValue == 'bassInfo'){
			pageUrl = '<c:out value="${wzwg_contextPath}${prefix}"/>/module/cntnts/selectCntntsBassInfoDetailAjax.do'
		}else if(paramValue == 'dataManage'){
			pageUrl = '<c:out value="${wzwg_contextPath}${prefix}"/>/module/cntnts/selectCntntsCnListAjax.do'
	...
	function fn_deleteCntntsCn(paramSeq){
		if(!confirm('<spring:message code="wzwg.cmm.cmmMsg.CMG007"><spring:argument><spring:message code="wzwg.cmm.word.delete" /></spring:argument></spring:message>')){
			...
			$.ajax({
		        type : 'POST'
				, url : '<c:out value="${wzwg_contextPath}${prefix}"/>/module/cntnts/deleteModuleCntntsCnAjax.do'
				, data : $("#cntntsFrm").serialize()
    ...
    function fn_modifyCntntsCnForm(paramSeq){
        ...
        if(cntntsVer == '1'){
	       $.ajax({
	        ...
	        var frm = document.cntntsFrm;
	        frm.action = "<c:out value='${wzwg_contextPath}${prefix}'/>/module/cntnts/registModuleCntntsCnFormAjax.do";
	...
	function fn_init(){
		...
		$.ajax({
	        type:'POST'
	      , url:'<c:out value="${wzwg_contextPath}${prefix}"/>/module/cntnts/selectCntntsCnListAjax.do'
	      , cache : false
	      , async : false
	      , data:$("#cntntsFrm").serialize()
	      , success:function (data) {
    ...
    function fn_registCntntsCnInit(){
        ...
        $.ajax({
              type : 'POST'
            , dataType: 'json'
            , url : '<c:out value="${wzwg_contextPath}${prefix}"/>/module/cntnts/registModuleCntntsCnTemplatInitAjax.do'
            , data:$("#cntntsFrm").serialize()
            , success:function (data) {
	...
	<form:form modelAttribute="paramVO" path="cntntsFrm" id="cntntsFrm" name="cntntsFrm" method="post" onsubmit="return false;">
	    ...
	    <div class="step wztab underLine theme-blue adminIcon">
	    	<ul class="tapMenu wztab-list">
				...
				<li class="wztab-item"><span class="ico-auth"></span><a href="javascript:void(0);" onclick="fnTabChange('author');" 		title="<spring:message code="wzwg.cmm.word.author" />" 		id="author" 		name="bbsTab"><spring:message code="wzwg.cmm.word.author" /></a></li>
			</ul>
		</div>
