...
<script>
	...
	function modifyQuickLinkAjax(){
		//var frm = $("#quickLinkForm")[0];
 		//var formData = new FormData(frm);
		if($('#qmenuTySe').val() == 'L'){
			if($('#quickLinkForm input[name="menuSttusCode"]:checked').val() == 'Y'){
				var viewAtCnt = 0; //노출 카운터
				var thisQmenuSeq = $('#quickLinkForm #qmenuSeq').val();
				$('#sideQuickItemList tr[data-viewat="true"]').each(function(){
					//console.log($(this).attr('data-qmenuSeq') + '/' + thisQmenuSeq);
					if($(this).attr('data-qmenuSeq') != thisQmenuSeq){
						//자신의 아이템은 제외하고 카운터 증가
						viewAtCnt++;
					}
				});
		...
		if($('#icon_file').val() != ''){
 		...
 		$("#quickLinkForm").ajaxSubmit({
	        type : 'POST'
			, url : '<c:out value="${wzwg_contextPath}" /><c:out value="${prefix}" />/module/sideQuick/modifySideQuickLinkAjax.do'
...
<form id="quickLinkForm" name="quickLinkForm" method="post" enctype="multipart/form-data">
	<input type="hidden" name="qmenuSeq" id="qmenuSeq" value="<c:out value="${paramVO.qmenuSeq }" />">
	<input type="hidden" name="menuImagePath" id="menuImagePath" value="<c:out value="${quickVO.menuImagePath }" />">
			...
			<tr>
				...
				<td>
					<select id="qmenuTySe" name="qmenuTySe" onchange="typeSeChange()">
						...
						<option value="D" <c:if test="${quickVO.qmenuTySe eq 'D' }">selected="selected"</c:if>><spring:message code="wzwg.module.word.segroupnm"/></option>
					</select>
				</td>
			</tr>
									...
									<c:if test="${empty oneDepth.upperMenuSeq or oneDepth.upperMenuSeq eq '0' }">
										<li>
										 <c:choose>
										 	...
										 	<c:otherwise>
										 		<a href="javascript:void(0);" onclick="selectMenuLink('/subList/<c:out value="${oneDepth.menuLinkSeq}" />','<c:out value="${oneDepth.menuNm }" />' )" data-href="menuLinkSeq" data-attr="menuNm"><c:out value="${oneDepth.menuNm }" /></a>	
										 	</c:otherwise>
										 </c:choose>
											<ul>
											...
											<c:if test="${oneDepth.menuSeq eq twoDepth.upperMenuSeq}">
												<li>
													<c:choose>
													 	...
													 	<c:otherwise>
													 		<a href="javascript:void(0);" onclick="selectMenuLink('/subList/<c:out value="${twoDepth.menuLinkSeq}" />','<c:out value="${twoDepth.menuNm }" />' )" data-href="menuLinkSeq" data-attr="menuNm"><c:out value="${twoDepth.menuNm }" /></a>	
													 	</c:otherwise>
													 </c:choose>
													...
													<c:forEach items="${menuList}" var="threeDepth" varStatus="status">
													  <c:if test="${twoDepth.menuSeq eq threeDepth.upperMenuSeq}"> 
														...
														<c:choose>
														 	...
														 	<c:otherwise>
														 		<a href="javascript:void(0);" onclick="selectMenuLink('/subList/<c:out value="${threeDepth.menuLinkSeq}" />','<c:out value="${threeDepth.menuNm }" />' )" data-href="menuLinkSeq" data-attr="menuNm"><c:out value="${threeDepth.menuNm }" /></a>	
														 	</c:otherwise>
														 </c:choose>
										...
										<li>
											<a href="javascript:void(0);" onclick="selectMenuLink('/cmm/mber/sbscrb/selectSbscrbCrtfc.do','<spring:message code="wzwg.cmm.word.signup" />' )" ><spring:message code="wzwg.cmm.word.signup" /></a>
										</li>
										<li>
											<a href="javascript:void(0);" onclick="selectMenuLink('/loginForm.do','<spring:message code="wzwg.cmm.word.login" />' )" ><spring:message code="wzwg.cmm.word.login" /></a>
										</li>
										<li>
											<a href="javascript:void(0);" onclick="selectMenuLink('/actionLogout.do','<spring:message code="wzwg.cmm.word.logout" />' )" ><spring:message code="wzwg.cmm.word.logout" /></a>
										</li>
										<li>
											<a href="javascript:void(0);" onclick="selectMenuLink('/searchIdForm.do','<spring:message code="wzwg.cmm.word.findi" />' )" ><spring:message code="wzwg.cmm.word.findi" /></a>
										</li>
										<li>
											<a href="javascript:void(0);" onclick="selectMenuLink('/searchPwForm.do','<spring:message code="wzwg.cmm.word.findp" />' )" ><spring:message code="wzwg.cmm.word.findp" /></a>
										</li>
										<li>
											<a href="javascript:void(0);" onclick="selectMenuLink('/sitemap.do','<spring:message code="wzwg.cmm.word.sitemap" />' )" ><spring:message code="wzwg.cmm.word.sitemap" /></a>
										</li>
										<li>
											<a href="javascript:void(0);" onclick="selectMenuLink('/cmm/mber/myPage/selectMyPageMain.do','<spring:message code="wzwg.cmm.word.youracct" />' )" ><spring:message code="wzwg.cmm.word.youracct" /></a>
										</li>
<%-- 										<li>
										...
										<li>
											<a href="javascript:void(0);" onclick="selectMenuLink('/module/stplatLog/selectStplatLogList.do?stplatSeq=<c:out value="${stplatList.stplatSeq }" />','<c:out value="${stplatList.stplatNm }" />' )" ><c:out value="${stplatList.stplatNm }" /></a>
										</li>
										</c:forEach>
									</ul>
						</div>
			...
			<tr class="linkFormData">
				<th><spring:message code="wzwg.module.word.linkadres"/></th>
				<td>
					<input type="text" class="w100" id="qmenuLinkUrl" name="qmenuLinkUrl" value="<c:out value="${quickVO.qmenuLinkUrl }" />">
			...
			<tr class="linkFormData">
				...
				<td>
					<ul class="wzForm">
						<li><label><input type="radio" name="qmenuTyCode" value="self" <c:if test="${quickVO.qmenuTyCode eq 'self' }">checked="checked"</c:if>><span class="spanLabel"><spring:message code="wzwg.cmm.word.nowwin"/></span></label></li>
						<li><label class="ml5"><input type="radio" name="qmenuTyCode" value="new" <c:if test="${quickVO.qmenuTyCode eq 'new' }">checked="checked"</c:if>><span class="spanLabel"><spring:message code="wzwg.cmm.word.newwin"/></span></label></li>
					</ul>
				</td>
			</tr>
			<tr>
				<th>
					<span class="linkTyHint"><spring:message code="wzwg.module.word.linksj"/></span>
					<span class="grpTyHint" style="display:none"><spring:message code="wzwg.module.word.segroupnm"/></span>
				</th>
				<td>
					<input type="text" class="w100" id="qmenuNm" name="qmenuNm" value="<c:out value="${quickVO.qmenuNm }" />">
			...
			<tr class="linkFormData">
				...
				<td>
					<c:set var="imgPath">/images/wzwg/site/mngr/no-img.png</c:set>
					<c:if test="${not empty quickVO.menuImagePath }"><c:set var="imgPath"><c:out value="${quickVO.menuImagePath }" /></c:set></c:if>
					<button type="button" id="btn_addIcon" style="border:1px solid #ddd"><img src="<c:out value="${imgPath }" />" id="icon_preview" onclick="$('#icon_file').click();" style="width: 80px;"></button>
			...
			<tr>
				...
				<td>
					<ul class="wzForm">
						<li><label><input type="radio" name="menuSttusCode" checked="checked" value="Y" <c:if test="${quickVO.menuSttusCode eq 'Y' }">checked="checked"</c:if>><span class="spanLabel"><spring:message code="wzwg.cmm.word.exposure" /></span></label></li>
						<li><label class="ml5"><input type="radio" name="menuSttusCode" value="N" <c:if test="${quickVO.menuSttusCode eq 'N' }">checked="checked"</c:if>><span class="spanLabel"><spring:message code="wzwg.cmm.word.unexposure" /></span></label></li>
					</ul>
				</td>
			</tr>
		</tbody>
	...
	<div class="rt-box">
		<button type="button" onclick="modifyQuickLinkAjax();" class="wzbtn btn-save bg"><spring:message code="wzwg.cmm.word.regist"/></button> 
