...
//	    $('#cntntsCn').val($('#cntntsEdit').html().replace(/\/con tents/g, '/contents'));
		...
		$.ajax({
	        type : 'POST'
			, url : '<c:out value="${wzwg_contextPath}${prefix}"/>/module/cntnts/registModuleCntntsCnAjax.do'
	...
	/* 레이아웃 추가 */
	 function addLayout(layBtnArea){
		 ...
		 if(containerClass.search('varLayout') >= 0){
			...
			$.ajax({
				   type:'POST'
				 , url:'<c:out value="${wzwg_contextPath}${prefix}"/>/module/cntnts/screen/selectSiteLayoutAjax.do'
				 , data:formData
				 , success:function (data) {
					 
					 wzAjaxModal('popup_l', '<spring:message code="wzwg.module.word.layoutadd" />', data);
					 	//$("#divLayerPopup").html(data);
			    	  	//$("#divLayerPopup").show();
			    	  	//$(".pop-box").toggle();
							 // 부모코드 셋팅 
	 ...
	 function addContentsPopup(contentsZone){
	 	 ...
	 	 $.ajax({
	 		   type:'POST'
	 		 , url:'<c:out value="${wzwg_contextPath}${prefix}"/>/module/cntnts/screen/selectLayoutContentsPopupAjax.do'
	 		 , cache : false
	 		 , async : false
	 		 , processData: false
	 		 , contentType: false
	 		 , data : form
	 		 , success:function (data) {
	 			 wzAjaxModal('popup_l', '<spring:message code="wzwg.module.word.cntntsadd" />', data);
	 			 	//$("#divLayerPopup").html(data);
	 	    	  	//$("#divLayerPopup").show();
	 	    	  	//$(".pop-box").toggle();
	 					 // 부모코드 셋팅 
	 ...
	 function addVarContentsPopup(btnContoller){
	 	...
	 	/*
	 	 ...
	 	 $.ajax({
	 		   type:'POST'
	 		 , url:'<c:out value="${wzwg_contextPath}${prefix}"/>/module/cntnts/screen/selectVariableContentsPopup.do'
	 		 , cache : false
	 		 , async : false
	 		 , processData: false
	 		 , contentType: false
	 ...
	 function fnGridBtnText(){
		if($('#styleGridNone').length == 0){
			$('#GridToggleBtn').html('<spring:message code="wzwg.module.word.guidancelinehide" />');
		}else{
			$('#GridToggleBtn').html('<spring:message code="wzwg.module.word.guidancelinex" />');
		}
	 }
	 ...
	 function deleteTabMenu(cntntsTabs){
		 if(confirm(wz_msg('wzwg.cmm.msg.screen.MSG085'))){
...
//		 var table = $('.tableTest').html();
		 ...
		 wzAjaxModal('popup_l', '<spring:message code="wzwg.module.word.tableedit" />', table);
		 ...
		 var atableOpt = {
	    		  ...
	    		  mark: {
	    			...
	    			,message: {
	    			    mergeCells: '<spring:message code="wzwg.module.word.celmerge" />',
			...
			<div class="mt5 mb5">
				...
				<div class="dgnControll">
					<div class="btnbox-c txt-r">
						<button type="button" class="wzbtn btn-del" onclick="screenReset()"><spring:message code="wzwg.module.word.alldelete" /></button>
						<button type="button" class="wzbtn btn-basic" onclick="" ><spring:message code="wzwg.module.word.mapmngr" /></button>
						<button type="button" class="wzbtn btn-basic" onclick="editContentCss()" >CSS <spring:message code="wzwg.cmm.word.edit" /></button>
						<button type="button" class="wzbtn btn-basic" onclick="fnGridToggle()" id="GridToggleBtn"><spring:message code="wzwg.cmm.word.grdtgl" /></button>
						<button type="button" class="wzbtn btn-basic" onclick="screenScrolView()"><spring:message code="wzwg.module.word.scrlview" /></button>
					</div>
				</div>
				...
				<div class="addContainer" id="addContainerZone">
					<div>
						<span class="wzbtn-group w100">
							<button type="button" class="wzbtn wzbtn-block btn-blue w40" style="padding: 6px;" onclick="addLayout($(this).parent().parent().parent())"><spring:message code="wzwg.module.word.layoutadd01" /></button>
							<button type="button" class="wzbtn wzbtn-block btn-orange w40" style="padding: 6px;" onclick="addTabContainer()"><spring:message code="wzwg.module.word.tabrelmadd" /></button>
						</span>
					</div>
				</div>
			</div>
			...
			<div id="md_addLayout" style="display: none;">
				<div class="addContainer" id="addLayoutZone">
					<div>
						<span class="wzbtn-group w100">
							<button type="button" class="wzbtn wzbtn-block btn-blue w40" style="padding: 6px;" onclick="addLayout($(this).parent().parent().parent())"><spring:message code="wzwg.module.word.layoutadd01" /></button>
							<button type="button" class="wzbtn wzbtn-block btn-black-bg w40" style="padding: 6px;" onclick="addVarContentsPopup($(this).parent().parent().parent())"><spring:message code="wzwg.module.word.cntntsadd01" /></button>
						</span>
					</div>
				</div>
			</div>
