...
try{document.title = $('#menuPath').val().replace(/>/gi,'-') + '<spring:message code="wzwg.cmm.word.regist" />';}catch(e){console.log(e.message);}
	...
	$(document).ready(function(){
		...
		$('#regist_btn').click(function(){
			...
			if(!confirm('<spring:message code="wzwg.cmm.cmmMsg.CMG007"><spring:argument><spring:message code="wzwg.cmm.word.stre" text="save" /></spring:argument></spring:message>')){
				...
				if(frm.mvpUrl.value == ''){
					alert('<spring:message code="wzwg.cmm.cmmMsg.CMG011"><spring:argument><spring:message code="wzwg.cmm.word.vdolnk" /></spring:argument><spring:argument><spring:message code="wzwg.cmm.word.input" /></spring:argument></spring:message>');
					return;				
				}

				if('<c:out value="${editorEstbsSe}"/>' == 'S'){
					frm.mvpCap.value = oEditors.getById["mvpCap"].getIR();	
					
					if(frm.mvpCap.value == ''){
						alert('<spring:message code="wzwg.cmm.cmmMsg.CMG011">'+
								'<spring:argument><spring:message code="wzwg.cmm.word.cap" text="contents" /></spring:argument>'+
								'<spring:argument><spring:message code="wzwg.cmm.word.input" text="input" /></spring:argument>'+
						      '</spring:message>');
						oEditors.getById["mvpCap"].exec("FOCUS",[]);
						return;				
					}
				}

				if('<c:out value="${editorEstbsSe}"/>' == 'C'){
					if(!bEditor_mvpCap.IsDirty()){ // 크로스에디터 안의 컨텐츠 입력 확인 
						alert('<spring:message code="wzwg.cmm.cmmMsg.CMG011">'+
								'<spring:argument><spring:message code="wzwg.cmm.word.cap" text="contents" /></spring:argument>'+
								'<spring:argument><spring:message code="wzwg.cmm.word.input" text="input" /></spring:argument>'+
						      '</spring:message>');
						bEditor_mvpCap.SetFocusEditor(); // 크로스에디터 Focus 이동 
						return false; 
					}
					
					frm.mvpCap.value = bEditor_mvpCap.GetBodyValue("mvpCap");
				}
				...
				$.ajax({
			        type : 'POST'
					, url : '<c:out value="${wzwg_contextPath}${prefix}"/>/module/ntt/mvp/registNttMvpInfoAjax.do'
		...
		$('#subospec_add_btn').click(function(){
			...
			frm.action = "<c:out value='${wzwg_contextPath}${prefix}'/>/module/bbs/mvp/selectBbsInc.do";
			frm.submit();
		});
	...
	function fnFileUploader(){
		...
		<form:form modelAttribute="paramVO" path="regFrm" id="regFrm" name="regFrm" method="post" enctype="multipart/form-data">
			...
			<c:set var="vdolnkTit"><spring:message code="wzwg.cmm.word.vdolnk" /></c:set>
			
			<div class="register-box">
				<div class="subject">
					<table>
					<caption><spring:message code="wzwg.module.word.postwritng" /></caption>
					<colgroup>
						<col width="10%"/>
						<col width="15%"/>
						<col width="13%"/>
					...
					<c:if test="${paramVO.subospecSeq ne null}">
						<tr>
							<th scope="row" class="subTit"><spring:message code="wzwg.cmm.word.ctgry02" /></th>
							<td colspan="4">
								<form:select path="subospecSeq" id="subospecSeq" title="${fn:escapeXml(ctgryTit)}">
								...
								<%-- <c:if test="${paramVO.mngrAt eq 'Y'}">
									<a href="javascript:void(0);" id="subospec_add_btn" class="wzbtn-table btn-srch"><spring:message code="wzwg.module.word.ctgryadd" /></a>	
								</c:if> --%>						
							</td>
						</tr>
					</c:if>	
						<tr>
							<th scope="row" class="subTit"><spring:message code="wzwg.cmm.word.sj" /></th>
						...
						<tr>
							...
							<td colspan="4">
								...
								<p class="admpg-subp w100 fl mt10"><span class="circle_no bg-green-strong">i</span><spring:message code="wzwg.cmm.msg.tip.MSG102" /></p>
							</td>
						</tr>
						</c:if>
						<tr>
							<th scope="row" class="subTit"><spring:message code="wzwg.module.word.youtubevido" /></th>
							<td colspan="4">
								<form:input path="mvpUrl" id="mvpUrl" class="w70" dir="required" title="${fn:escapeXml(vdolnkTit)}" />
								<p class="admpg-subp w100 fl mt10"><span class="circle_no bg-green-strong">i</span><spring:message code="wzwg.cmm.msg.tip.MSG103" /></p>
							</td>
						</tr>
						<tr>
							<th scope="row" class="subTit"><spring:message code="wzwg.cmm.word.cap" /></th>
							<td colspan="4">
								<textarea name="mvpCap" id="mvpCap" rows="20" style="width:100%;display:none;"><c:out value="${paramVO.mvpCap}"/></textarea>
								
								<c:import url="${wzwg_contextPath}/module/editor/editorForm.do" charEncoding="utf-8">
									<c:param name="param_editorNm" 	value="mvpCap" />
									<c:param name="param_editorTy" 	value="custom" />
								</c:import>						
							</td>
						</tr>					
						<tr>
							<th scope="row" class="subTit"><spring:message code="wzwg.cmm.word.cn" /></th>
							<td colspan="4">
						...
						<%-- <tr>
							...
							<td colspan="4">
								<ul class="setlist">
									<li><label><form:checkbox path="answerPermAt" name="answerPermAt" value="N" dir="required" /> <span><spring:message code="wzwg.module.word.answerperm" /></span></label></li>
								</ul>
							</td>
						</tr>	 --%>
						<tr>
