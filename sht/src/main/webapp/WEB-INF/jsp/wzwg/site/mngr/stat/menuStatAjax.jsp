...
<script>
    $(function () {
    	...
    	var labels = [
    		<c:forEach items="${statList}" var="resultList" varStatus="status">'<c:out value="${fn:replace(fn:replace(resultList.menuNm, cn, \"\"), CR, \"\")}" escapeXml="true"/>' <c:if test="${!status.last}">,</c:if></c:forEach>
			];
		var data = {
				  labels: labels,
				  datasets: [
				    {
				      label: '<spring:message code="wzwg.site.stat.msg.MSG003" />',
				      data: [
		...
		var config = {
				  ...
				  options: {
				    ...
				    plugins: {
				      ...
				      title: {
				        display: true,
				        text:  '<spring:message code="wzwg.cmm.word.wzwg.site.stat.msg.MSG006" /> (<spring:message code="wzwg.site.stat.msg.MSG005" />)'
				      }
				    }
				  },
				};
